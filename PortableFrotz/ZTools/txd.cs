...
namespace ZTools
{
    ...
    {
        public static System.Collections.Generic.List<String> instructions;
        public static System.Collections.Generic.Dictionary<int, String> opcodes;
        ...
        static int decode_operands(String opcode_name, int par1, int par2, int par3, int par4, int extra, int type)
        {
            ...
            else
            {
                ...
                if (txio.option_inform)
                {
                    ...
                    for (i = 0; i < len; i++)
                        txio.tx_printf("{0}", opcode_name[i]);
                }
                else
                {
                    if (!opcodes.ContainsKey(opcode.opcode))
                    {
                        opcodes.Add(opcode.opcode, opcode_name);
                    }

                    if (!instructions.Contains(opcode_name))
                    {
                        instructions.Add(opcode_name);
                    }
