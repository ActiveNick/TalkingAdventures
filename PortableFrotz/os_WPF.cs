...
namespace Frotz
{
    ...
    {
        public static event EventHandler ReadLineStarted;
        internal static void OnReadLineStarted()
        {
            if (ReadLineStarted != null) ReadLineStarted(null, EventArgs.Empty);
        }
        ...
        public static Blorb.BlorbFile _blorbFile = null; // TODO Make this static again, or something
...
#endif
            // TODO Find a way to make a beep
        }
         ...
         *
         */
        public static bool read_file_name(out System.IO.Stream stream, string default_name, FileTypes flag)
        {
            System.IO.Stream s = null;
            switch (flag)
            {
                case FileTypes.FILE_SAVE:
                    s = _screen.OpenNewOrExistingFile(FastMem.save_name, "Choose save game file", "Save Files (*.sav)|*.sav", ".sav");
                    break;
                case FileTypes.FILE_RESTORE:
                    s = _screen.OpenExistingFile(FastMem.save_name, "Choose save game to restore", "Save Files (*.sav)|*.sav");
                    break;
                case FileTypes.FILE_SCRIPT:
                    s = _screen.OpenNewOrExistingFile(General.DEFAULT_SCRIPT_NAME, "Choose Script File", "Script File (*.scr)|*.scr", ".scr");
                    break;
                case FileTypes.FILE_RECORD:
                    s = _screen.OpenNewOrExistingFile(default_name, "Choose File to Record To", "Record File(*.rec)|*.rec", ".rec");
                    break;
                case FileTypes.FILE_PLAYBACK:
                    s = _screen.OpenExistingFile(default_name, "Choose File to playback from", "Record File(*.rec)|*.rec");
                    break;
         ...
         *
         */
        public static bool process_arguments(String _filename)
        {
            byte[] filedata;
            String fileName = null;
            fileName = _screen.SelectGameFile(_filename, out filedata);
            if (fileName != null)
            {
        ...
        public static zword read_line(int max, zword[] buf, int timeout, int width, bool continued)
        {
            ...
            _screen.SetInputMode(true, true, false);
            ...
            try
            {
                while (true)
                {
                    ...
                    // System.Diagnostics.Debug.WriteLine("Entries:" + entries.Count);
                    ...
                    {
                        c = entries.Dequeue();
                        System.Diagnostics.Debug.WriteLine("CHAR:" + c + ":" + (char)c);
                    }
                    ...
                    else if (c == CharCodes.ZC_RETURN)
                    {
                        ...
                        if (entries.Count > 0)
                        {
                            System.Diagnostics.Debug.WriteLine("Have more entries than I should:" + entries.Count);
                        }
            ...
            finally
            {
                ...
                _screen.SetInputMode(false, false, false);
        ...
        public static zword read_key(int timeout, bool cursor)
        {
            ...
            _screen.SetInputMode(true, true, true);
            ...
            {
                _screen.SetInputMode(false, false, true);
            }
        }
        ...
        public static void restart_game(int stage)
        {
            ...
            if (stage == ZMachine.RESTART_BEGIN)
            {
                _screen.Restarting();
            }
        }
        ...
        {
            // TODO Maybe I can find a better place to handle changing more_prompts
            bool shouldWrap = _screen.ShouldWrap();
            Frotz.Generic.Screen.more_prompts = shouldWrap;
            if (shouldWrap == true)
            {
                return 1;
            }
        ...
        {
            // TODO Put the hook here to restore all the memory from the z-machine, then as soon as it hits in

            _screen.StoryStarted(main.story_name, _blorbFile, main.h_version);
        }
    ...
    [DebuggerDisplay("Char = {CharString}")]
    internal class BufferChars
    {
        internal char CharString { get; private set; }
